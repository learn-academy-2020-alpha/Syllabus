...
The View /app/views/users/show.json.jbuilder is JBuilder:
```ruby
json.id @user.id
json.name @user.name
...
The View /app/views/users/index.json.jbuilder needs to be updated as well so that we show the image there:
```ruby
